...
- Migration task: para reduzir falhas de rede durante bootstrap, `run-leads-migrations-task.sh` executa a task Fargate em subnets publicas com `assignPublicIp=ENABLED` e security group interno. A task e temporaria; os servicos continuam em subnets privadas.
- Logs de migration: o script procura o log stream real no CloudWatch por task id e imprime os eventos em sucesso ou falha. Deploy sem logs deve ser tratado como falha de infraestrutura/log delivery, nao como erro SQL invisivel.
