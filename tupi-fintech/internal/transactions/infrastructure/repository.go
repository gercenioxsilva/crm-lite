...
func (r *PgCommandRepository) Create(tx domain.Transaction) error {
	_, err := r.db.Exec(context.Background(),
		`INSERT INTO transactions (id, account_id, amount, currency, created_at, status, reason)
		 VALUES ($1,$2,$3,$4,$5,$6,$7)`,
		tx.ID, tx.AccountID, tx.Amount, tx.Currency, tx.CreatedAt, tx.Status, tx.Reason,
	)
	return err
}

func (r *PgQueryRepository) FindByID(id domain.TransactionID) (domain.Transaction, error) {
	var t domain.Transaction
	row := r.db.QueryRow(context.Background(),
		`SELECT id, account_id, amount, currency, created_at, status, reason FROM transactions WHERE id = $1`, id,
	)
